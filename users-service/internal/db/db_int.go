...
type DB interface {
	...
	GetUsers(limit, offset int64) ([]*models.User, error)
...
type TX interface {
	...
	GetFriendRequestByUserIds(senderId uuid.UUID, receiverId uuid.UUID) (*models.FriendRequest, error)
	CreateFriendRequest(request *models.FriendRequest) error
	UpdateFriendRequest(request *models.FriendRequest) error
	DeleteFriendRequest(id uuid.UUID) error
	GetFriendRequest(id uuid.UUID) (*models.FriendRequest, error)
	GetUserLink(id uuid.UUID) (*models.UserLink, error)
	DeleteUserLink(id uuid.UUID) error
}
